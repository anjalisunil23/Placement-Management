...
const OfficerApi = {
  ...
  mapApplication(a) {
    ...
    const certificates = (Array.isArray(a.certificates) ? a.certificates : []).map((c, i) => {
      ...
      return {
        ...
        url: `${API_BASE}/officer/applications/${encodeURIComponent(id)}/certificates/${encodeURIComponent(index)}`,
      };
    });
    return {
      id,
  ...
  async fetchPendingSelfPlacements(params = {}) {
    ...
    if (params.q) qs.set('q', params.q);
    const q = qs.toString();
    const res = await api('/officer/students/self-placement/pending' + (q ? `?${q}` : ''));
    if (!res.success || !Array.isArray(res.data)) return null;
    return res.data.map(s => OfficerApi.mapStudentRow(s));
  },
