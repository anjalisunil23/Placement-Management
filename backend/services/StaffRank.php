 ...
 *
 * Prefer an AES numeric field when present; otherwise derive from designation text.
 */
final class StaffRank
{
    ...
    public static function pickNumeric(array $source): ?int
    {
        $keys = [
            ...
            'rank_no', 'rankNo', 'staff_level', 'staffLevel', 'rank',
        ];
        foreach ($keys as $key) {
        ...
        // Shallow nested bags AES sometimes wraps profile fields in.
    ...
    public static function fromDesignation(string $designation): int
    {
        ...
        // Full professor (not associate / assistant / of practice).
        ...
        if (preg_match('/\b(LECTURER|INSTRUCTOR|DEMONSTRATOR|TECHNICAL|LAB\s*ASSISTANT|ADJUNCT|PROFESSOR\s+OF\s+PRACTICE)\b/', $d)) {
            return 6;
        }
